...
export function Table({ children, className = "" }: { children: ReactNode; className?: string }) {
  return (
    <div className="overflow-hidden rounded-xl border border-white/[0.02] bg-acp-card backdrop-blur-md shadow-premium-card w-full">
...
export function Th({ children, className = "", ...props }: { children: ReactNode } & ThHTMLAttributes<HTMLTableCellElement>) {
  return (
    <th 
      className={`px-6 py-4 text-left text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400 select-none border-b border-white/5 bg-[#161B22] sticky top-0 z-10 ${className}`}
      {...props}
    >
      {children}
    </th>
