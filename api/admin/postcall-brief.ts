  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      if (action === "generate") {
        ...
        if (hasInngest()) {
          const emitRes = await emitEvent("brief/postcall_requested", {
            briefId: recordId,
            dealId,
            schemaId: activeSchemaId,
            notes: notes || "",
          ...
          console.warn("[Postcall Brief] Inngest was active but emitEvent failed. Falling back to synchronous processing.");
        }
